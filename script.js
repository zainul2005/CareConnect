...
function showNavbarLocation() {
    const loc = document.getElementById("navLocation");
    loc.textContent = "📍 Detecting location...";

    if (!navigator.geolocation) {
        loc.textContent = "📍 Location not supported";
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        async (pos) => {
            ...
            try {
                ...
                loc.textContent = `📍 ${city}, ${state}`;
            } catch (err) {
                loc.textContent = "📍 Location detected";
            }
        },
        () => {
            loc.textContent = "📍 Enable location in browser";
        },
        {
            timeout: 10000
        }
...
document.addEventListener("DOMContentLoaded", showNavbarLocation);
